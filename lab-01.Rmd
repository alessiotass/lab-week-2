...
**NA stands for not available, so the dataset does not have any data for that specific date or attraction or region etc. Thus when it tries to find the median or mean, it cannot calculate it**
...
**By default, na.rm is set to false in the code. Thus when you are summarising data, you must set it to true by inputting na.rm = TRUE into your code**
...
**When the mean for data is higher than the median, this shows that there are a few attractions with far more viewers than the rest, which skews the data to have a higher mean.For example, a simple dataset with 5 numbers : "10,10,10,100,200" would have a mean of 66, but a median of 10.**
